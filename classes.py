...
#   ------------------Helper Functions------------------
    ...
    def __check_existence(self, name):
        self._db.execute('''SELECT * FROM habits WHERE HabitName = ? AND User = ? ''', (name, self._username))
    ...
    def create(self, name, period, entry_date = date.today()):
        ...
        if period == "Daily" or period == "Weekly":    
            ...
            #INSERT the new habit into the database
                ...
                #INSERT Statement
                self._db.execute('''INSERT INTO habits VALUES(NULL, ?, ?, ?, ?, 0, 0, 0)''', (name, period, self._username, entry_date))
                #Commit the changes
    ...
    def delete(self, name):
        ...
        if self.__check_existence(name):
            ...
            #DELETE Statement
            self._db.execute('''DELETE FROM habits WHERE HabitName = ? AND User = ?''', (name, self._username))
    ...
    def check(self, name, entry_date = date.today()):
        ...
        if self.__check_existence(name):
            ...
            #Check if an data entry for today already exists
             self._db.execute('''SELECT * FROM trackingdata INNER JOIN habits ON habits.HabitID = trackingdata.HabitID
            ...
            #If the habit is already checked return an error message
             if exists:
                ...
                self._db.execute('''SELECT HabitID FROM habits WHERE HabitName = ? AND User = ?''', (name, self._username))
                ...
                self._db.execute('''SELECT Period FROM habits WHERE HabitName = ? AND User = ?''', (name, self._username))
                ...
                self._db.execute('''SELECT MAX(CheckDate) FROM trackingdata WHERE HabitID = ?''', (habit_id,))
                ...
                if not last_date:
                    self._db.execute('''UPDATE habits SET CurrentStreak = CurrentStreak + 1 WHERE HabitID = ?''', (habit_id,))
                ...
                #If the habit already has tracking data, check if the streak is interrupted
                    ...
                    if period == "Daily":
                        ...
                        #If the date difference is > 1 set the streak to zero
                            ...
                            self._db.execute('''UPDATE habits SET CurrentStreak = 0 WHERE HabitID = ?''', (habit_id,))
                            #Increase the breaks by one
                            self._db.execute('''UPDATE habits SET Breaks = Breaks + 1 WHERE HabitID = ?''', (habit_id,))
                ...
                self._db.execute('''SELECT CurrentStreak FROM habits WHERE HabitID = ?''', (habit_id,))
                #Extract the value from the result
                current_streak = self.__extract_value()

                #Query the longest streak
                self._db.execute('''SELECT LongestStreak FROM habits WHERE HabitID = ?''', (habit_id,))
                #Extract the value from the result
                longest_streak = self.__extract_value()

                #UPDATE the value if the current streak is the longest streak
                if current_streak > longest_streak:
                    self._db.execute('''UPDATE habits SET LongestStreak = ? WHERE HabitID = ?''', (current_streak, habit_id))
                
                #INSERT a new entry in the tracking data table
                self._db.execute('''INSERT INTO trackingdata VALUES(NULL, ?, ?)''',  (entry_date, habit_id))      
